...
模型加载器始终显示内置模型选择：官方 Full、官方 Dev-2604，以及转换后的 Full/Dev BF16、FP16、FP8。如果选中的模型已经在本地存在，就直接使用本地文件夹。如果本地不存在，开启 `download_if_missing` 后会把选中的模型下载到 `ComfyUI/models/diffusion_models`。
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `base_model_name` | `HiDream-O1-Image-BF16` | Full O1 权重；也可选择官方 `HiDream-O1-Image` |
...
- 官方 Full 模型：[HiDream-ai/HiDream-O1-Image](https://huggingface.co/HiDream-ai/HiDream-O1-Image)
- 官方 Dev 2604 模型：[HiDream-ai/HiDream-O1-Image-Dev-2604](https://huggingface.co/HiDream-ai/HiDream-O1-Image-Dev-2604)
