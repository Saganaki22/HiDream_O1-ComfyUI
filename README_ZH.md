...
模型加载器始终显示六个内置模型选择：Full/Dev 的 BF16、FP16、FP8。如果选中的模型已经在本地存在，就直接使用本地文件夹。如果本地不存在，开启 `download_if_missing` 后会把选中的模型下载到 `ComfyUI/models/diffusion_models`。
...
| Dev | `FlashFlowMatchEulerDiscreteScheduler` | 自定义 Euler，带噪声缩放，按少步数蒸馏模型调校 |
